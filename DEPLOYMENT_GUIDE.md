...
| `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, `VAPID_SUBJECT` | send-push. The public key is also a constant in `src/lib/push/pushService.ts` (no `VITE_` variable). | send-push returns 503 and sends nothing. There are no code fallbacks. | Rotated 2026-09-14 after the old private key was found committed. To rotate again: `npx web-push generate-vapid-keys`, set all three secrets, update the constant, deploy send-push, then push the frontend. Browsers with permission re-subscribe automatically on their next visit (`ensureSubscriptionUsesCurrentKey`). |

...
- **SRM Portal Fee & Finance: `#tbl7` (`ids=7`) is a gross billing clearance ledger, NOT student payments.** The ERP clears charges via scholarships/waivers (`Student Concession`), internal adjustments, and student bank receipts. Summing `#tbl7` reports gross billing clearances (e.g. ₹14,77,860) rather than actual student out-of-pocket payments (e.g. ₹6,28,060). Always parse individual payment receipts from `students/report/receiptgeneration.jsp` (`ids=27&stuId=...`) with authentic receipt numbers (`SEAS/...`). Institutional scholarship concessions must be stored with `paid_amount = 0.00` and `receipt_number = SCHOLARSHIP-<term>` so they are displayed as credits/waivers and never inflate out-of-pocket totals.
- **Academic calendar & holiday resolution:** Never ask an LLM to interpret compressed working-day grids or calculate weekday offsets on its own. Always resolve relative dates (today, tomorrow, coming weekdays like "coming monday", explicit dates) deterministically through `public.get_calendar_day(p_date)` backed by `academic_calendar_days` and inject as `RESOLVED_FACTS`.

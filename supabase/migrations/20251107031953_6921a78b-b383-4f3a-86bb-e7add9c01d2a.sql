-- Fix update_verification_status function to properly handle mobile field and all mentor data
...
BEGIN
  ...
  IF new_status = 'approved' THEN
    ...
    -- Extract mobile from application_data
    v_mobile := verification_data->>'mobile';
    
    -- Insert or update mentor record with all fields including mobile
    ...
    VALUES (
      target_user_id, 
      COALESCE(verification_data->>'name', user_data.name),
      ...
      COALESCE(verification_data->>'profile_image', user_data.profile_image),

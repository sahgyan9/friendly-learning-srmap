...
- **Status at end**:
  ...
  - **Production follow-up, same day, with owner approval:** deleted `test-send-email`, `test-gemini-keys`, `test-gemini-pool`, `populate-mentors` (all now 404). Deployed contact-form v139, send-email-queue v20, import-srm-portal v24, sync-srm-portal v19, ai-chatbot v173, generate-ai-overview v39, parse-linkedin-pdf v26; `verify_jwt` unchanged for each. Verified over HTTP without writes: contact-form returns 400 for a 6,000-char message and for a bad email, 405 for GET; send-email-queue 401 for anon; embed-knowledge 401 for a wrong secret. `platform_settings.ai_overview_enabled` is `true`, so the switch fix changes nothing visible.
- **Next agent should**:
  - Done later the same day: Gyan applied `20260914120000` in the SQL editor (job active, `*/10`, reads Vault, secret present); embed-knowledge deployed as v18 and the old literal now gets 401; `LOVABLE_API_KEY` unset.
  - **VAPID key rotated (approved by Gyan):** the old pair was hardcoded in send-push and no VAPID secrets were set. New secrets set (subject = site URL), send-push deployed without fallbacks (anon call returns 401, not the 503 it gives when unconfigured), public key is now a constant in `pushService.ts`, and browsers with permission move to the new key automatically on their next visit. Commit `46f1190`. `VITE_VAPID_PUBLIC_KEY` in Vercel is no longer read and can be deleted.
  - **Missing RPCs confirmed** (PostgREST PGRST202 for both, and the SQL editor shows no pg_proc rows): `set_user_admin_status`, `get_related_searches`. Restore migrations `20260914130000` (RPC plus is_admin guard trigger, idempotent, in the harness) and `20260914130100` (related searches, hardened, SKIP-listed) were applied by Gyan in the SQL editor the same day. Verified over PostgREST: `set_user_admin_status` now exists (anon gets 42501 permission denied, as intended) and `get_related_searches` returns suggestions (200). Types regenerated; the last casts on those calls are removed; typecheck 0.
  - Also: `f74d4cc` briefly added 8 typecheck errors (test file without vitest imports); fixed in `65c54e9`.

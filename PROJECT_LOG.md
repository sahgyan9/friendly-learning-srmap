...
- **Next agent should**:
  ...
  - **VAPID private key is public:** no VAPID secrets are set, so send-push signs with the key pair hardcoded in its source. Rotate (new pair, set VAPID_* secrets and Vercel `VITE_VAPID_PUBLIC_KEY`, remove the fallbacks, redeploy send-push); existing push subscriptions will need re-enabling. Needs owner approval.

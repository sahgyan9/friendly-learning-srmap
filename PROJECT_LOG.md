...
- **Next agent should**:
  ...
  - **Missing RPCs confirmed** (PostgREST PGRST202 for both, and the SQL editor shows no pg_proc rows): `set_user_admin_status`, `get_related_searches`. Restore migrations `20260914130000` (RPC plus is_admin guard trigger, idempotent, in the harness) and `20260914130100` (related searches, hardened, SKIP-listed) are committed; apply them in the SQL editor, then verify both RPCs over PostgREST, regenerate types and drop the two remaining casts in admin.ts and related-searches.ts.
